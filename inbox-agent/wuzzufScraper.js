...
async function scrapeWuzzuf() {
  ...
  try {
    ...
    const searchUrl = 'https://wuzzuf.net/search/jobs/?q=backend&a=hpb';
    console.log(`   └ Navigating to: ${searchUrl}`);
    await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });

    // Wuzzuf job cards are usually wrapped in standard HTML
    // We will extract all links that point to a job posting
    ...
    console.log(`   └ Extracted ${jobCards.length} raw jobs from Wuzzuf.`);
    jobs.push(...jobCards);
